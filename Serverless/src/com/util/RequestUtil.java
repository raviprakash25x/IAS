...
public class RequestUtil 
{
	...
	/**
	 * contains a mapping from request Id->Thread
	 */
	...
	public JSONObject forward_request(JSONObject container) throws Exception
	{
    	...
    	String req_id= (String)container.get("request_id");
    	container.put("queue", "loadbalancer");
    	container.put("type", "service_request");
    	
		if(!serviceName.equalsIgnoreCase("logging"))
			messageObject.logMessage("INFO", "Request came for "+serviceName+" with request id "+req_id);
		
		requestThMap.put(req_id, Thread.currentThread());
		System.out.println("Message sent");
		messageObject.sendMessage(container);
		JSONObject message=getMessage(req_id);
		
		if(!serviceName.equalsIgnoreCase("logging"))
			messageObject.logMessage("INFO", "Response came for "+serviceName+" with request id "+req_id);
		return message;
	}
	...
	public JSONObject getMessage(String x) throws Exception
	{
		 ...
		 * can only have one thread executing inside them at the same time
		 */
		synchronized (Thread.currentThread()) 
		{
			/*
			 * Once msg is received on the RMQ receiving
			 * queue listener, notifies the thread corresponding
			 * to reqd ID, hence the wait() gets over.
			 * Also response map is set after listening there itself
			 */
			Thread.currentThread().wait();
			response=responseMap.get(x);
			/*
			 * once the response is received for the request,
			 * remove it from req and respose maps
			 */
